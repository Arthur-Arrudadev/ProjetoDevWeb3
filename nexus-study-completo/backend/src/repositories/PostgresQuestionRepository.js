...
async updateResults(data) {
	...
	async markReview({ user_id, question_id, discipline }) {
		await pool.query(
			`INSERT INTO results (user_id, discipline, total_questions, correct, wrong, to_review, evaluated_at)
			 VALUES ($1, $2, 1, 0, 0, 1, CURRENT_DATE)
			 ON CONFLICT (user_id, discipline, evaluated_at)
			 DO UPDATE SET
			   to_review      = results.to_review + 1,
